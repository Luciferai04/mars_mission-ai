...
class TestResourceOptimizer:
    ...
    def test_power_optimization_exceeds_budget(self):
        ...
        assert len(optimized) < len(activities) or any(
            a.get("status") == "deferred" for a in optimized
        )
    ...
    def test_thermal_management_cold(self):
        ...
        activities = [{"id": "act1", "type": "imaging", "duration_min": 20}]

        environment = {"temperature_c": -50}

        adjusted = optimizer.thermal_management(
            activities, environment, operating_temp_min_c=-40, operating_temp_max_c=40
        )
...
class TestConstraintSolver:
    ...
    def test_evaluate_time_exceeded(self, mock_activities):
        ...
        activities = [{"id": f"act{i}", "duration_min": 200} for i in range(5)]
    ...
    def test_evaluate_power_exceeded(self):
        ...
        activities = [{"id": "high_power", "power_w": 1000, "duration_min": 10}]

        constraints = {"power": {"mmrtg_output_w": 110, "peak_demand_w": 900}}
    ...
    def test_repair_adds_missing_comm(self):
        ...
        activities = [{"id": "science", "type": "imaging", "duration_min": 20}]
    ...
    def test_evaluate_no_activities(self):
        ...
        assert not result["ok"]
        assert any(
            "no activities" in v.lower() or "missing" in v.lower()
...
class TestIntegration:
    ...
    def test_resource_optimizer_with_path_planner(self, mock_activities):
        ...
        # Optimize resources
        optimized, summary = optimizer.optimize_power_usage(
            mock_activities, mmrtg_output_w=110.0, time_budget_min=480
        )
