...
def mock_rover_state():
    """Mock rover state."""
    return {
        "location": {"lat": 18.4447, "lon": 77.4508},
        "sol": 1400,
        "battery_soc": 0.85,
        "temperature_c": -60,
...
def mock_activities():
    ...
    return [
        ...
        {"id": "post_tx", "type": "data_tx", "duration_min": 15, "power_w": 180},
    ]
...
def mock_nasa_traverse_response():
    ...
    return {
        ...
        "features": [
            {
                "type": "Feature",
                "geometry": {"type": "Point", "coordinates": [77.4508, 18.4447]},
                "properties": {"sol": 1400, "site": 142, "drive": 2056},
            }
        ],
    }
...
def sample_dem_file(temp_dir, mock_dem_data, mock_dem_metadata):
    ...
    np.savez_compressed(dem_path, elevation=mock_dem_data, metadata=mock_dem_metadata)
    return dem_path
