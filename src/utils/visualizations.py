...
def plot_dem_with_route(elevation: np.ndarray,
    ...
    if route_path:
        ...
        plt.plot(route_array[:, 1], route_array[:, 0], 
...
def plot_slope_map(slope: np.ndarray,
                   hazard_threshold: float = 30.0,
                   output_path: Optional[str] = None) -> None:
...
def plot_mission_timeline(activities: List[Dict[str, Any]],
                          output_path: Optional[str] = None) -> None:
...
def plot_power_profile(activities: List[Dict[str, Any]],
    ...
    """
    # Build power profile
    max_time = max([a.get('start_min', 0) + a.get('duration_min', 0) 
    ...
    ax.axhline(y=mmrtg_power, color='r', linestyle='--', label=f'MMRTG Output ({mmrtg_power}W)')
...
def plot_hazard_statistics(hazards: Dict[str, float],
                           output_path: Optional[str] = None) -> None:
...
def plot_resource_summary(power_summary: Dict[str, float],
                         time_budget: int,
                         total_time: int,
                         output_path: Optional[str] = None) -> None:
    ...
    axes[0].bar(['Used', 'Remaining'], 
               [power_used, power_remaining],
               color=['orange', 'green'])
...
def generate_mission_visualizations(plan: Dict[str, Any],
