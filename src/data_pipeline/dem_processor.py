...
class DEMProcessor:
    ...
    def compute_slope(
        self, elevation: np.ndarray, pixel_size: Tuple[float, float]
    ) -> np.ndarray:
        ...
        self.logger.info(
            f"Slope computed: min={slope_deg.min():.2f}°, max={slope_deg.max():.2f}°, mean={slope_deg.mean():.2f}°"
        )
        return slope_deg
    ...
    def identify_hazards(
        ...
        self.logger.info(
            f"Hazard analysis: {hazards['safe_percentage']:.1f}% safe, "
    ...
    def compute_and_cache_slope(
        self, dem_path: str, max_safe_slope: float = 30.0
    ) -> Dict[str, Any]:
