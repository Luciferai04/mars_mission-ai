...
class NaturalLanguageQueryInterface:
    ...
    def _generate_plan_from_nl(self,
                               query: str,
...
class ConversationalInterface:
    ...
    def chat(self, user_message: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        enriched_context['conversation_history'] = self.conversation_history[-5:]  # Last 5 messages
    ...
    def _format_plan_as_response(self, plan: Dict[str, Any]) -> str:
        ...
        for step in plan.get('steps', []):
            response += f"{step['step_number']}. {step.get('description', step['action'])}\n"
