...
class MSRSampleCachingSystem:
    ...
    def plan_sample_caching_strategy(
        ...
        cache_sites = self._identify_cache_locations(traverse_plan, science_targets)
    ...
    def execute_sample_collection(
        self, sample_target: Dict[str, Any], sol: int, mission: MSRMission
    ) -> Sample:
        ...
        """
        self.logger.info(
            f"Collecting sample at {sample_target['location']} on Sol {sol}"
        )
        ...
        self.logger.info(
            f"Sample {sample.sample_id} collected and sealed (Tube {sample.tube_number})"
        )

        return sample

    def execute_cache_deployment(
        self,
        ...
        """
        self.logger.info(
            f"Deploying cache at {cache_location} with {len(samples_to_cache)} samples"
        )
        ...
        for sample in samples_to_cache:
            ...
            if success:
                ...
                self.logger.info(
                    f"Sample {sample.sample_id} cached at {cache.cache_id}"
                )
            else:
        ...
        self.logger.info(
            f"Cache {cache.cache_id} deployed with {len(cache.current_samples)} samples"
        )

        return cache

    def _identify_cache_locations(
        self, traverse_plan: Dict[str, Any], targets: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _allocate_samples_to_caches(
        self,
        mission: MSRMission,
    ...
    def _generate_caching_schedule(
        self,
        mission: MSRMission,
    ...
    def _plan_backup_caches(
        self, primary_caches: List[Dict[str, Any]], allocation: Dict[str, List[str]]
    ) -> List[Dict[str, Any]]:
        ...
        for primary in primary_caches:
            ...
            backup_loc = (primary_loc[0] + 0.001, primary_loc[1] + 0.001)  # ~60m offset
    ...
    def _plan_sample_retrieval(
        ...
        prioritized = sorted(
            all_caches, key=lambda c: c.get("accessibility_score", 0.5), reverse=True
        )
            ...
            retrieval_sequence.append(
                {
                    ...
                    "estimated_sfr_sol": (
                        mission.retrieval_lander_eta_sol + i
                        if mission.retrieval_lander_eta_sol
                        else None
                    ),
