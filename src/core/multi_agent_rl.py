...
class RouteAgent(BaseAgent):
    ...
    def compute_reward(self, state: AgentState, action: Action,
        ...
        if len(state.science_targets) > 0:
            ...
            old_dist = np.sqrt((state.position[0] - target['lat'])**2 +
                             (state.position[1] - target['lon'])**2)
            new_dist = np.sqrt((next_state.position[0] - target['lat'])**2 +
...
class PowerAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.action_names = ['full_power', 'nominal', 'conservative',
                            'minimal', 'recharge']
    ...
    def compute_reward(self, state: AgentState, action: Action,
        ...
        if (len(next_state.completed_objectives) > len(state.completed_objectives) and
...
class ScienceAgent(BaseAgent):
    ...
    def compute_reward(self, state: AgentState, action: Action,
        ...
        target_types = set([obj.split('_')[0] for obj in next_state.completed_objectives])
...
class StrategyAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.action_names = ['aggressive_exploration', 'systematic_survey',
                            'targeted_investigation', 'sample_collection',
                            'conservative_ops', 'opportunistic']
    ...
    def compute_reward(self, state: AgentState, action: Action,
        ...
        completion_rate = len(next_state.completed_objectives) / max(len(state.science_targets), 1)
        reward += completion_rate * 100.0
        
        # Bonus for efficient multi-sol planning
        if next_state.current_sol > state.current_sol:
            objectives_per_sol = len(next_state.completed_objectives) / next_state.current_sol
...
class MultiAgentRLSystem:
    ...
    def __init__(self, models_dir: str = "models/marl"):
    ...
    def _weighted_action_selection(self, agent_actions: Dict, agent_q_values: Dict) -> int:
    ...
    def train_episode(self, initial_state: AgentState, max_steps: int = 100) -> float:
        ...
        for step in range(max_steps):
            ...
            for name, agent in self.agents.items():
                ...
                agent.update(state_vector, action_idx, agent_reward,
    ...
    def _simulate_step(self, state: AgentState, action: Action) -> Tuple[AgentState, float, bool]:
        """Simulate environment step (simplified simulation)"""
        
        # Create next state
        next_state = AgentState(
            position=action.target if action.target else state.position,
            battery_soc=max(0, state.battery_soc - action.power_required / 2064.0),  # 2064Wh capacity
        ...
        if action.action_type == 'drive' and len(state.science_targets) > 0:
            target = state.science_targets[0]
            dist = np.sqrt((next_state.position[0] - target['lat'])**2 +
                         (next_state.position[1] - target['lon'])**2)
        ...
        # Calculate reward
        reward = sum(agent.compute_reward(state, action, next_state)
                    for agent in self.agents.values()) / len(self.agents)
        
        # Check if done
        done = (next_state.battery_soc < 0.05 or
                next_state.time_remaining <= 0 or
                len(next_state.science_targets) == 0)
    ...
    def save_all_agents(self):
        ...
        for name, agent in self.agents.items():
            path = self.models_dir / f"{name}_agent.json"
            agent.save(str(path))
    ...
    def load_all_agents(self):
        ...
        for name, agent in self.agents.items():
            ...
            if path.exists():
                agent.load(str(path))
                logger.info(f"Loaded {name} agent from {path}")
    
    def get_training_stats(self) -> Dict[str, Any]:
        """Get training statistics"""
        return {
            'episodes': self.training_episodes,
            'avg_reward': np.mean(self.collective_rewards[-100:]) if self.collective_rewards else 0,
            'total_experiences': sum(len(agent.replay_buffer) for agent in self.agents.values()),
            'agent_epsilons': {name: agent.epsilon for name, agent in self.agents.items()},
            'recent_rewards': self.collective_rewards[-10:] if self.collective_rewards else []
        }
    ...
    def optimize_mission_plan(self, mission_context: Dict) -> Dict:
        ...
        state = AgentState(
            position=(mission_context.get('lat', 18.4), mission_context.get('lon', 77.4)),
        ...
        return {
            ...
            'rl_confidence': np.mean([1 - agent.epsilon for agent in self.agents.values()])
        }
...
def train_marl_system(episodes: int = 1000, save_interval: int = 100):
    ...
    for episode in range(episodes):
        ...
        initial_state = AgentState(
            position=(18.4 + np.random.randn() * 0.01,
                     77.4 + np.random.randn() * 0.01),
            battery_soc=0.7 + np.random.rand() * 0.2,
            time_remaining=480,
            hazard_map=np.random.rand(100, 100),
            science_targets=[{'id': f'target_{i}', 'lat': 18.4 + np.random.randn() * 0.02,
                            'lon': 77.4 + np.random.randn() * 0.02} for i in range(5)],
            current_sol=1597,
