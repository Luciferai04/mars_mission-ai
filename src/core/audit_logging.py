...
class AuditLogger:
    ...
    def _setup_persistent_logging(self):
        ...
        log_file = self.log_directory / f"audit_{datetime.utcnow().strftime('%Y%m%d')}.jsonl"
    ...
    def log_event(self,
        ...
        log_message = f"[{event_type.value}] {action} | Resource: {resource} | Actor: {actor_id}"
    ...
    def validate_resource(self,
                         ...
                         resource_type: str,
                         validator_id: str,
                         checks: List[Dict[str, Any]]) -> ValidationRecord:
        ...
        resource_checks = [c for c in checks if 'resource' in c.get('category', '').lower()]
        science_checks = [c for c in checks if 'science' in c.get('category', '').lower()]
        ...
        validation = ValidationRecord(
            ...
            resource_constraints_validated=all(c.get('passed', False) for c in resource_checks),
            science_objectives_validated=all(c.get('passed', False) for c in science_checks),
            confidence_score=sum(c.get('confidence', 1.0) for c in checks) / len(checks) if checks else 0.0
        )
    ...
    def get_event_chain(self, event_id: str) -> List[AuditEvent]:
        ...
        # Find starting event
        start_event = next((e for e in self.event_cache if e.event_id == event_id), None)
        if not start_event:
            return chain
        ...
        while current:
            ...
            if current.parent_event_id:
                current = next((e for e in self.event_cache if e.event_id == current.parent_event_id), None)
            else:
                break
    ...
    def generate_traceability_report(self,
        ...
        # Get approval status
        approval_events = [e for e in events if e.event_type in [
            EventType.PLAN_APPROVED, EventType.PLAN_REJECTED
        ]]
        ...
        report = {
            ...
            'validation': {
                ...
                'checks_passed': len([c for c in validation.checks_performed if c.get('passed')]) if validation else 0,
                'checks_failed': len(validation.issues_found) if validation else 0,
                'nasa_requirements': {
                    'safety_validated': validation.safety_validated if validation else False,
                    'resource_constraints_validated': validation.resource_constraints_validated if validation else False,
                    'science_objectives_validated': validation.science_objectives_validated if validation else False
            ...
            'approval': {
                ...
                'approver': approval_events[-1].approver_id if approval_events else None,
                'approval_timestamp': approval_events[-1].approval_timestamp if approval_events else None
            },
            
            'integrity': {
                'checksums_verified': all(e.checksum == e.compute_checksum() for e in events),
