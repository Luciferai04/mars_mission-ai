...
class DEMAutoDownloader:
    ...
    def download_dem_for_region(self,
                                region_name: str,
                                lat_min: float,
                                lat_max: float,
                                lon_min: float,
        ...
        """
        self.logger.info(f"Requesting DEM for {region_name}: "
        ...
        if not products:
            ...
            return self._generate_fallback_dem(region_name, lat_min, lat_max, lon_min, lon_max)
        ...
        self._update_metadata(
            region_name, dem_path, best_product,
    ...
    def download_perseverance_traverse_dems(self,
                                           traverse_plan: Dict[str, Any],
                                           buffer_km: float = 5.0) -> List[Dict[str, Any]]:
    ...
    def auto_update_dems(self, max_age_days: int = 30) -> List[Dict[str, Any]]:
        ...
        for region_name, meta in self.metadata.items():
            downloaded_at = datetime.fromisoformat(meta.get('downloaded_at', '2000-01-01'))
    ...
    def _query_available_products(self,
                                  lat_min: float,
                                  lat_max: float,
                                  lon_min: float,
                                  lon_max: float,
        ...
        try:
            pds_products = self._query_pds_geosciences(lat_min, lat_max, lon_min, lon_max)
    ...
    def _query_usgs_ode(self,
                        lat_min: float,
                        lat_max: float,
                        lon_min: float,
                        lon_max: float) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            products = []
            for item in data.get('ODEResults', {}).get('products', {}).get('product', []):
                products.append({
    ...
    def _query_pds_geosciences(self,
                               lat_min: float,
                               lat_max: float,
                               lon_min: float,
                               lon_max: float) -> List[Dict[str, Any]]:
        ...
        self.logger.debug("PDS Geosciences query not fully implemented - using fallback")
        return []
    
    def _query_hirise(self,
                     lat_min: float,
                     lat_max: float,
                     lon_min: float,
                     lon_max: float) -> List[Dict[str, Any]]:
    ...
    def _select_best_product(self,
                            products: List[Dict[str, Any]],
                            target_resolution_m: float) -> Dict[str, Any]:
    ...
    def _check_cache(self,
                     ...
                     lat_max: float,
                     lon_min: float,
                     lon_max: float) -> Optional[Dict[str, Any]]:
        ...
        if (abs(bounds.get('lat_min', 0) - lat_min) < 0.01 and
            abs(bounds.get('lat_max', 0) - lat_max) < 0.01 and
            abs(bounds.get('lon_min', 0) - lon_min) < 0.01 and
            abs(bounds.get('lon_max', 0) - lon_max) < 0.01):
