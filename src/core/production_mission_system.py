...
from .msr_sample_caching import MSRSampleCachingSystem, MSRMission
...
class ProductionMissionSystem:
    ...
    def process_natural_language_request(
        ...
        result = {"query": query, "plan": plan, "context": context, "executed": False}
    ...
    def plan_multi_sol_mission(
        ...
        mission_plan = self.multi_sol_planner.plan_multi_sol_mission(
            ...
            constraints={
                ...
                or f"Mission_{datetime.utcnow().strftime('%Y%m%d')}",
                "start_sol": 0,
            },
        )
    ...
    def analyze_terrain_imagery(
        ...
        """
        self.logger.info(
            f"Analyzing {len(image_paths)} {camera_type} images from Sol {sol}"
        )
    ...
    def plan_sample_caching_strategy(
        self, mission_plan: MultiSolMissionPlan, science_targets: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        if not self.current_mission:
            self.current_mission = MSRMission(mission_id=mission_plan.mission_name)
    ...
    def execute_contingency_plan(
        ...
        matching = [
            c for c in current_plan.contingency_plans if c.get("trigger") == hazard_type
        ]
    ...
    def generate_mission_report(
        self, mission_plan: MultiSolMissionPlan, include_traceability: bool = True
    ) -> str:
        """
        ...
        report.append("=" * 80)
        report.append("NASA MARS MISSION PLANNING REPORT")
        ...
        report.append("-" * 80)
        report.append(
            f"Mission Duration: Sol {mission_plan.start_sol} to {mission_plan.end_sol} ({mission_plan.num_sols} sols)"
        )
        report.append(f"Total Traverse Distance: {mission_plan.total_distance_m:.1f} m")
        report.append(
            f"Science Targets: {mission_plan.high_priority_targets_visited} high-priority"
        )
        ...
        for sol_plan in mission_plan.sol_plans:
            ...
            report.append(
                f"  Location: {sol_plan.start_location} → {sol_plan.end_location}"
            )
            report.append(
                f"  Drive: {sol_plan.drive_distance_m:.1f} m ({sol_plan.drive_time_hours:.1f} hrs)"
            )
            report.append(
                f"  Power: {sol_plan.power_consumed_wh:.1f} Wh (margin: {sol_plan.power_margin_wh:.1f} Wh)"
            )
            ...
            if sol_plan.contingency_triggers:
                report.append(
                    f"    Contingencies: {', '.join(sol_plan.contingency_triggers)}"
                )
        ...
        if include_traceability:
            ...
            trace = self.audit_logger.generate_traceability_report(
                mission_plan.mission_name, "mission_plan"
            )

            report.append(f"  Events Logged: {trace['lifecycle']['total_events']}")
            report.append(f"  Validated: {trace['validation']['validated']}")
            report.append(
                f"  Safety Validated: {trace['validation'].get('nasa_requirements', {}).get('safety_validated', False)}"
            )
            report.append(
                f"  Approval Required: {trace['approval']['approval_required']}"
            )
    ...
    def _get_mission_context(self) -> Dict[str, Any]:
        ...
        try:
            ...
            context = {
                ...
                "traverse": traverse,
                "current_mission": (
                    vars(self.current_mission) if self.current_mission else None
                ),
            }
    ...
    def _ensure_dems_for_mission(
        self, start_location: tuple, targets: List[Dict[str, Any]]
    ):
    ...
    def _validate_mission_plan(
        self, mission_plan: MultiSolMissionPlan
    ) -> Dict[str, Any]:
        ...
        checks.append(
            {
                ...
                "passed": all(
                    sp.power_margin_wh >= 200 for sp in mission_plan.sol_plans
                ),
                "confidence": 1.0,
            }
        )
        ...
        checks.append(
            {
                ...
                "passed": all(
                    sp.drive_distance_m <= 150 for sp in mission_plan.sol_plans
                ),
                "confidence": 1.0,
            }
        )
        ...
        checks.append(
            {
                ...
                "passed": mission_plan.total_power_consumed_wh
                <= (900 * mission_plan.num_sols),
                "confidence": 1.0,
            }
        )
    ...
    def _generate_emergency_contingency(
        self, hazard_type: str, plan: MultiSolMissionPlan, state: Dict[str, Any]
    ) -> Dict[str, Any]:
...
def create_production_system(
    openai_api_key: Optional[str] = None, nasa_api_key: Optional[str] = None
) -> ProductionMissionSystem:
    """Create production mission system with sensible defaults."""
    return ProductionMissionSystem(
        openai_api_key=openai_api_key, nasa_api_key=nasa_api_key
    )
